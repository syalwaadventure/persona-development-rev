...
description: >-
  ...
  dan cara perusahaan atau usaha di Indonesia menghasilkan laba. Juga
  menyusun analisis arus kas, matriks IRR/return investasi, dan analisis
  peluang bisnis baru.
---
...
- analisis arus kas (cash flow) suatu perusahaan atau proyek;
- perhitungan/matriks IRR, NPV, atau kelayakan investasi suatu proyek/inisiatif;
- analisis peluang bisnis baru (business opportunity analysis).
...
**Catatan batas untuk IRR/arus kas:** skill ini menghitung dan menjelaskan IRR, NPV, dan arus kas pada level **proyek atau inisiatif bisnis** (mis. kelayakan proyek EPC, ekspansi pabrik, investasi mesin) untuk kebutuhan pemahaman model bisnis dan business opportunity — bukan valuasi saham emiten atau rekomendasi beli/jual. Bila pengguna memintanya untuk saham, jelaskan batas ini dan arahkan sesuai §3 di atas.
...
- Cash flow: struktur arus kas operasi, investasi, dan pendanaan;
  jembatan dari profit (accrual) ke kas riil; kebutuhan modal kerja.
- IRR/Investment Return Matrix: IRR, NPV, payback period, dan matriks
  sensitivitas (skenario × discount rate) untuk kelayakan proyek/inisiatif.
- Business Opportunity Analysis: penilaian peluang bisnis baru dari sisi
  daya tarik pasar, kelayakan, kesesuaian strategis, risiko, dan indikasi
  return.
...
### 6a. Auto-deteksi Analysis Mode (mengurangi ketergantungan pada prompt panjang)

Skill tidak perlu menunggu pengguna menyebut nama mode secara eksplisit. Deteksi mode dari kata kunci berikut dan langsung jalankan mode yang cocok — cukup konfirmasi singkat di awal jawaban (satu kalimat), bukan pertanyaan balik:

| Kata kunci di permintaan pengguna | Mode yang dijalankan |
|---|---|
| "arus kas", "cash flow", "kas operasi/investasi/pendanaan" | Cash Flow Analysis |
| "IRR", "NPV", "payback period", "kelayakan investasi", "tingkat pengembalian" | Investment Return / IRR Matrix |
| "peluang bisnis", "opportunity", "ide bisnis baru", "ekspansi ke" | Business Opportunity Analysis |
| "model bisnis", "BMC", "business model canvas" | Business Model Canvas |
| "proses bisnis", "alur kerja", "SOP" | Business Process Map |
| "struktur organisasi", "job desk" | Organization and Job-Desk Map |
| Tidak ada kata kunci spesifik dan hanya menyebut nama perusahaan | Company Overview (default) |

Klarifikasi hanya diajukan bila: (a) permintaan bisa dibaca sebagai dua mode berbeda sekaligus dan hasilnya akan sangat berbeda, atau (b) nama perusahaan/industri sama sekali tidak disebut. Selain itu, langsung kerjakan dengan asumsi yang dinyatakan di awal jawaban (mis. "Saya asumsikan ini untuk analisis kelayakan proyek, bukan valuasi saham — beri tahu bila keliru.").
...
3. Susun elemen sesuai framework yang relevan:
   ...
   - Arus kas & IRR/kelayakan investasi →
     `references/irr-cashflow-guide.md`
   - Peluang bisnis baru → `references/business-opportunity-guide.md`
...
15. Cash Flow Analysis → `assets/cash-flow-template.md`
16. Investment Return / IRR Matrix → `assets/irr-matrix-template.md`
17. Business Opportunity Analysis → `assets/business-opportunity-template.md`
...
- mengarang proses, struktur organisasi, job desk, revenue, biaya, atau
  profit;
- mengarang atau mengestimasi tanpa dasar: arus kas, IRR, NPV, discount
  rate, atau asumsi payback period — semua angka proyeksi wajib
  ditandai `AGENT INFERENCE`/`REQUIRES COMPANY VALIDATION` beserta dasar
  asumsinya (lihat `references/irr-cashflow-guide.md`);
- menyatakan inferensi atau praktik umum industri sebagai fakta resmi
  perusahaan tertentu;
- memberikan rekomendasi investasi atau menyimpulkan "layak dijalankan"
  atas nama pengguna — skill menyajikan angka dan sensitivitasnya,
