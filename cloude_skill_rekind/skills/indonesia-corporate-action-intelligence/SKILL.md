...
description: >-
  ...
  untuk daily brief, tracking, analisis event, dan pemeriksaan rumor. Juga
  menyusun Rekind Sector News Brief — ringkasan berita harian sektor Energi,
  Oil & Gas, Petrokimia, dan Pupuk dalam format HTML siap baca, dengan
  estimasi waktu baca dan alat bantu visual.
---
...
### 2a. Rekind Sector News Brief — perluasan cakupan terbatas

Selain aksi korporasi, skill ini juga menyusun **ringkasan berita sektor harian** khusus untuk kebutuhan pemantauan lingkungan bisnis Rekind (lihat Mode K, §6, dan `references/rekind-sector-brief-guide.md`). Ini satu-satunya mode di skill ini yang boleh memuat berita non-aksi-korporasi (kebijakan, harga komoditas, proyek, regulasi sektor), karena tujuannya radar harian, bukan verifikasi satu event.

Batasan yang tetap berlaku pada mode ini:

- Setiap butir berita tetap mencantumkan sumber dan tanggal.
- Rumor tetap ditandai sebagai rumor, bukan fakta.
- Skill tetap tidak memberi rekomendasi investasi.

Yang berbeda dari mode lain: kedalaman verifikasi per butir lebih ringan (cukup satu sumber kredibel per butir, tidak wajib silang-verifikasi ke sumber resmi seperti event corporate action material), karena fungsinya adalah radar, bukan keputusan.
...
**Auto-deteksi Mode K (Rekind Sector News Brief).** Skill tidak perlu menunggu instruksi lengkap untuk mode ini. Bila permintaan pengguna cocok dengan salah satu pola berikut, langsung jalankan Mode K memakai parameter default di `references/rekind-sector-brief-guide.md` §2, tanpa bertanya balik:

- Frasa pemicu: "news brief", "sector brief", "brief sektor", "berita hari ini", "update sektor", "berita energi/migas/oil and gas/petrokimia/pupuk hari ini", atau permintaan terjadwal harian yang tidak menyebut nama perusahaan/aksi korporasi tertentu.
- Tidak menyebut nama perusahaan spesifik, jenis aksi korporasi, atau mode lain secara eksplisit.

Klarifikasi hanya diajukan bila pengguna secara eksplisit meminta sektor di luar empat sektor default, atau bila permintaan bisa dibaca sebagai Mode K maupun mode lain (mis. "brief BUMN energi hari ini" bisa berarti Mode K atau Daily Brief korporasi — tanyakan singkat).
...
| Sector Watch | 10–20 |
| Weekly Digest | 12–25 |
| Rekind Sector News Brief (Mode K) | 8–16 (2–4 per sektor) |
...
| K. Rekind Sector News Brief | Ringkasan berita harian sektor Energi, Oil & Gas, Petrokimia, Pupuk — format HTML | `assets/rekind-sector-news-brief-template.html`, `references/rekind-sector-brief-guide.md` |

### 6a. Mode K — Rekind Sector News Brief

Mode ini berbeda dari mode A–J dalam tiga hal: cakupan beritanya lebih luas dari aksi korporasi (§2a), formatnya HTML (bukan markdown), dan alurnya dibuat seotomatis mungkin (§4, auto-deteksi Fase 1) supaya pengguna tidak perlu menulis prompt panjang setiap hari.

**Sektor default (tidak perlu diminta ulang setiap kali):** Energi & Ketenagalistrikan, Migas (Oil & Gas), Petrokimia, Pupuk/Fertilizer. Detail keyword pencarian per sektor ada di `references/rekind-sector-brief-guide.md` §1.

**Sumber:** ikuti hierarki tiga tier di `references/source-guide.md`, dengan **CNBC Indonesia** sebagai salah satu sumber utama Tier 2 yang diprioritaskan untuk berita sektor energi, migas, petrokimia, dan pupuk — di samping Kontan, Bisnis Indonesia, Katadata, dan media bisnis kredibel lain. Sumber resmi (Kementerian ESDM, Kementerian BUMN, IDX, situs resmi emiten sektor terkait) tetap dipakai untuk memverifikasi angka atau kebijakan yang disebut material.

**Format output:** HTML tunggal (satu file, CSS & JS inline), mengikuti struktur `assets/rekind-sector-news-brief-template.html`:

1. **Header** — judul, tanggal, periode (default 24 jam terakhir), dan **estimasi waktu baca** (target 6–8 menit; cara hitung di `references/rekind-sector-brief-guide.md` §3).
2. **Ringkasan poin penting** — 3–5 butir tersegmentasi, ditampilkan sebagai kotak visual (bukan paragraf panjang), untuk pembaca yang hanya punya waktu scan cepat.
3. **Bagian per sektor** — satu blok per sektor yang punya berita hari itu (lewati sektor tanpa berita, jangan dipaksakan). Tiap butir: judul singkat, 1–2 kalimat inti, sumber + tanggal, tag dampak potensial terhadap Rekind (mis. harga gas, kebijakan pupuk bersubsidi, proyek EPC baru).
4. **Tabel ringkas** — semua butir berita dalam satu tabel scan-cepat: Sektor · Perusahaan/Pihak · Inti Berita · Sumber · Tanggal · Relevansi (LOW/MEDIUM/HIGH untuk Rekind).
5. **Alat bantu visual** — minimal satu elemen visual (bar sederhana untuk jumlah berita per sektor, atau badge warna untuk tingkat relevansi). Jangan memakai chart yang butuh library eksternal; pakai CSS/SVG murni supaya file tetap mandiri.
6. **Sumber & keterbatasan** — daftar sumber yang diperiksa dan yang tidak dapat diakses, singkat.
7. **Disclaimer standar** di footer.

**Kurasi, bukan agregasi mentah.** Jangan menampilkan semua hasil pencarian; pilih butir yang relevan dengan bisnis Rekind (EPC, energi, migas, petrokimia, pupuk), buang duplikasi dan berita yang tidak menyentuh sektor tersebut meski katanya "trending". Maksimal 3–5 butir per sektor kecuali sedang ada perkembangan besar.
...
| `references/rekind-sector-brief-guide.md` | Fase 1, 2 & 8 — Mode K: sektor default, keyword, waktu baca, kurasi |
...
| `assets/rekind-sector-news-brief-template.html` | Fase 8 — Mode K |
